...
def additem(item):
    try:
        ...
        insertitem = insert(items).values(
            userId=session.get('UID'), item=item, role="user")
        logger.info("insertitem:{}".format(insertitem))
...
def delitem(ID):
    try:
        ...
        deleteitem = delete(items).where(
            and_(items.c.ID == ID, items.c.userId == session.get('UID')))
...
def createUser(fname, lname, uname, email, password):
    ...
    try:
        ...
        insertuser = insert(usersData).values(
            FName=fname, LName=lname, UName=uname, Email=email, Password=convertedpassword)
...
def gettaskbyid(uid, todo):
    ...
    try:
        ...
        result = db.query(items).filter(
            and_(items.c.userId == uid, items.c.ID == todo)).first()
